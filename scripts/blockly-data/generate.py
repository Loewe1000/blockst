...
# Every language the Scratch side ships gets a Blockly locale too, from
# Blockly's own msg/json translations (127 languages exist there).
LANGUAGES = sorted(path.stem for path in (HERE.parent / "scratchblocks-wasm" / "data" / "locales").glob("*.toml"))

DOWNLOADS = {
    **{f"blocks_{name}.ts": f"{BLOCKLY_RAW}/blocks/{name}.ts" for name in BLOCK_FILES},
    **{f"msg_{lang}.json": f"{BLOCKLY_RAW}/msg/json/{lang}.json" for lang in LANGUAGES},
...
def fetch(offline: bool) -> None:
    ...
    for name, url in DOWNLOADS.items():
        ...
        try:
            ...
            if error.code == 404 and name.startswith("msg_"):
                print(f"    no such file upstream, skipped")
                continue
            raise
...
def write_locale(
    ...
    """`full=False` writes only what differs per language — texts, markers,
    mouth labels, aliases — and leaves shapes, categories and slots to the
    base locale it inherits."""
    lines = [
        f"# {title}",
    ...
    icons = {k: ICONS[k] for k in sorted(specs) if k in ICONS} if full else {}
    if icons:
    ...
    aliases = {a: b for a, b in aliases.items() if a not in (markers or {}).values()}
    if aliases:
        lines += ["", "[aliases]"]
...
def standard_specs(definitions: list[dict], messages: dict[str, str], lang: str) -> tuple[dict, dict, list[str]]:
    """The Blockly standard blocks in one language: block_specs() plus the
    blocks that need a hand — the operators as blocks of their own, the
    while/until loop, the number-property test. German keeps its hand-set
    wording ("wiederhole [solange v]", "%1 ist %2"); the other languages
    build the while/until loop from Blockly's own option texts and skip the
    property test, whose predicate lives entirely in a dropdown."""
    specs, gaps = block_specs(definitions, messages)
    operator_aliases: dict[str, str] = {}
    for base, (shape, category, table) in OPERATORS.items():
        specs.pop(base, None)
        for suffix, (symbol, spellings) in table.items():
            block_id = f"{base}_{suffix}"
            specs[block_id] = {"text": f"%1 {symbol} %2", "shape": shape, "category": category, "slots": ["value", "value"], "inline": True}
            for spelling in spellings:
                operator_aliases[f"%1 {spelling} %2"] = block_id
    specs.pop("logic_operation", None)
    for suffix, key in (("and", "LOGIC_OPERATION_AND"), ("or", "LOGIC_OPERATION_OR")):
        word = messages.get(key)
        if word:
            # the logic operation stacks its two values on two rows, the word on the second
            specs[f"logic_operation_{suffix}"] = {"text": f"%1 {word} %2", "shape": "boolean", "category": "logik", "slots": ["value", "value"], "inline": False}
    mouth = messages.get("CONTROLS_REPEAT_INPUT_DO", "do")
    if lang == "de":
        specs["controls_whileUntil"] = {"text": "wiederhole %1 %2", "shape": "c-block", "category": "schleifen", "slots": ["dropdown", "value"], "mouth": mouth}
        specs["math_number_property"] = {"text": "%1 ist %2", "shape": "boolean", "category": "mathe", "slots": ["value", "dropdown"], "inline": True}
    else:
        specs.pop("controls_whileUntil", None)
        for suffix, key in (("while", "CONTROLS_WHILEUNTIL_OPERATOR_WHILE"), ("until", "CONTROLS_WHILEUNTIL_OPERATOR_UNTIL")):
            word = messages.get(key)
            if word:
                specs[f"controls_whileUntil_{suffix}"] = {"text": f"{clean(word)} %1", "shape": "c-block", "category": "schleifen", "slots": ["value"], "mouth": mouth}
        specs.pop("math_number_property", None)
    return specs, operator_aliases, gaps


def scratch_end_marker(lang: str) -> str:
    """The end marker the Scratch locale of that language uses, so `ende`,
    `fin`, `konec` … read the same across block languages."""
    path = HERE.parent / "scratchblocks-wasm" / "data" / "locales" / f"{lang}.toml"
    if path.exists():
...
def write_blockly_locales_rs(locales: list[str]) -> None:
    """The include_str! list the plugin embeds, one line per generated file."""
    path = HERE.parent / "scratchblocks-wasm" / "src" / "generated" / "blockly_locales.rs"
    lines = [
        "// Generated by scripts/blockly-data/generate.py — do not edit by hand.",
...
def main() -> int:
    ...
    print("Blockly standard blocks, other languages")
    language_report: list[str] = []
    generated: list[str] = []
    en_messages = with_synonyms(load_messages_json(SOURCES / "msg_en.json"))
    en_specs, en_aliases, en_gaps = standard_specs(definitions, en_messages, "en")
    write_locale(
        OUT / "locales" / "blockly-en.toml",
        "Blockly standard blocks, English (current wording).",
        "Text from Blockly (Apache-2.0), msg/json/en.json.",
        en_specs,
        markers={"scratchblocks:end": "end", "control_else": en_messages.get("CONTROLS_IF_MSG_ELSE", "else")},
        aliases={"end": "scratchblocks:end", "End": "scratchblocks:end", **en_aliases},
    )
    generated.append("blockly-en")
    language_report.append(f"| en | {len(en_specs)} | {len(en_gaps)} |")
    for lang in LANGUAGES:
        if lang in ("de", "en"):
            continue
        path = SOURCES / f"msg_{lang}.json"
        if not path.exists() or path.stat().st_size == 0:
            language_report.append(f"| {lang} | — | no msg/json file |")
            continue
        messages = with_synonyms(load_messages_json(path))
        specs, lang_aliases, gaps = standard_specs(definitions, messages, lang)
        end_word = scratch_end_marker(lang)
        write_locale(
            OUT / "locales" / f"blockly-{lang}.toml",
            f"Blockly standard blocks, {lang} (current wording).",
            f"Text from Blockly (Apache-2.0), msg/json/{lang}.json.",
            specs,
            inherits="blockly-en",
            markers={"scratchblocks:end": end_word, "control_else": messages.get("CONTROLS_IF_MSG_ELSE", "else")},
            aliases={end_word: "scratchblocks:end", end_word.capitalize(): "scratchblocks:end", **lang_aliases},
            full=False,
        )
        generated.append(f"blockly-{lang}")
        language_report.append(f"| {lang} | {len(specs)} | {len(gaps)} |")
    write_blockly_locales_rs(generated)
    print(f"  {len(generated)} locales")
